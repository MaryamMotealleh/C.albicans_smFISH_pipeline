...
def group_experiments_sd(root_dir, channel):
    mask_folder = f"{root_dir}/Masks"
    spot_folder = f"{root_dir}/Spots"
    dense_folder = f"{root_dir}/Spots decomposition"
    experiments = {}
    ...
    for file in glob(f'{root_dir}/*'):
        ...
        if match:
            identifier, channel_pos, exp_number = match[0]
            exp_number = int(exp_number)
            ...
            if not exp_number in experiments[identifier]:
                experiments[identifier][exp_number] = {}
            experiments[identifier][exp_number][channel_pos] = file
            ...
            # this will break if there are spots from more than one channel
            spot_candidate = glob(f"{spot_folder}/*{identifier}_{channel}_{exp_number:02d}*spots*")
            if spot_candidate:
                experiments[identifier][exp_number]['spots'] = spot_candidate[0]
...
def translate(image: Image, viewer: napari.Viewer, stepsize: int=1):
    ...
    def left(viewer):
        ...
        x=viewer.layers[name].translate[1]-stepsize
        viewer.layers[name].translate=(y,x)
        
    @viewer.bind_key('s', overwrite=True)
    def down(viewer):
        y=viewer.layers[name].translate[0]+stepsize
        x=viewer.layers[name].translate[1]
        viewer.layers[name].translate=(y,x)
